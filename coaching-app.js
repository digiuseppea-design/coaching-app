...
import { getAuth, GoogleAuthProvider, signInWithRedirect, getRedirectResult, onAuthStateChanged, signOut } from "https://www.gstatic.com/firebasejs/11.6.0/firebase-auth.js";
...
const firebaseConfig = {
  apiKey: "AIzaSyAzL4HxMrV-euk61zlo4ipcqcUYWuiJwMc",
  authDomain: "trainer-tascabile.firebaseapp.com",
  projectId: "trainer-tascabile",
  storageBucket: "trainer-tascabile.firebasestorage.app",
  messagingSenderId: "464307021985",
  appId: "1:464307021985:web:bb66f7cfb46a00a5d6e8e0"
};
...
const loginLoad   = document.getElementById('login-loading');
...
onAuthStateChanged(auth, async user => {
  if (user) {
    hideLogin();
    // Aggiungi bottoni logout e sync nella topbar-right
    if (!document.getElementById('btn-logout')) {
      const right = document.querySelector('.topbar-right');

      const btnSync = document.createElement('button');
      btnSync.id = 'btn-sync-all';
      btnSync.className = 'btn-ghost';
      btnSync.style.cssText = 'height:24px;font-size:9px;font-family:"DM Mono",monospace;letter-spacing:0.06em;color:var(--muted);flex-shrink:0;border:1px solid var(--border);border-radius:3px;padding:0 8px;white-space:nowrap;';
      btnSync.textContent = '☁ Sync';
      btnSync.title = 'Carica tutti i dati su Firestore';
      btnSync.onclick = syncAllToFirestore;
      right.prepend(btnSync);

      const btnLogout = document.createElement('button');
      btnLogout.id = 'btn-logout';
      btnLogout.className = 'btn-ghost';
      btnLogout.style.cssText = 'height:24px;font-size:9px;font-family:"DM Mono",monospace;letter-spacing:0.06em;color:var(--muted);flex-shrink:0;white-space:nowrap;';
      btnLogout.textContent = '⎋';
      btnLogout.title = 'Esci (' + (user.email||'') + ')';
      btnLogout.onclick = () => signOut(auth);
      right.prepend(btnLogout);
    }
    // Sync iniziale: carica da Firestore → localStorage
    await syncFromFirestore(user.uid);
    // Ricarica dati nell'app
    loadSheet();
    if (typeof restoreProgram === 'function') restoreProgram();
...
async function syncFromFirestore(uid) {
  try {
    const keys = ['coaching_clienti', 'coaching_pagamenti', 'coaching_saved_programs', 'coaching_sheet_data'];
    ...
    // Programma corrente
    const progSnap = await getDoc(doc(db, 'users', uid, 'data', 'coaching_program'));
    if (progSnap.exists()) {
      const val = progSnap.data().value;
      if (val) localStorage.setItem('coaching_program', typeof val === 'string' ? val : JSON.stringify(val));
    }
    console.log('✓ Sync da Firestore completato');
  } catch(e) {
    console.warn('Sync Firestore fallito, uso localStorage locale', e);
  }
}
...
async function saveToFirestore(key, value) {
  ...
  if (!user) return;
  try {
    await setDoc(doc(db, 'users', user.uid, 'data', key), { value, updatedAt: Date.now() });
  } catch(e) {
    console.warn('Salvataggio Firestore fallito per', key, e);
  }
}

// ── INTERCETTA localStorage PER SYNC AUTOMATICO ───────────────────────────────
const SYNC_KEYS = ['coaching_clienti','coaching_pagamenti','coaching_saved_programs','coaching_program','coaching_sheet_data'];
const _origSetItem = localStorage.setItem.bind(localStorage);
localStorage.setItem = function(key, value) {
  _origSetItem(key, value);
  if (SYNC_KEYS.includes(key) && auth.currentUser) {
    saveToFirestore(key, value);
  }
};

// ── SYNC COMPLETO localStorage → FIRESTORE ────────────────────────────────────
async function syncAllToFirestore() {
  ...
  try {
    for (const key of SYNC_KEYS) {
      ...
      if (val) await saveToFirestore(key, val);
    }
    if (btn) { btn.textContent = '✓ Sincronizzato'; btn.style.color = 'var(--accent)'; }
    setTimeout(() => {
      if (btn) { btn.textContent = '☁ Carica tutto su Firestore'; btn.disabled = false; btn.style.color = ''; }
    }, 2500);
  } catch(e) {
